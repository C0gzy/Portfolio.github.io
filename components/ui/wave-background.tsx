...
export function Waves({
    className = "",
    strokeColor = "#FFA940", // hex for hsl(30, 100%, 65%)
    ...
    const setLines = () => {
        ...
        // Add overflow to ensure waves extend beyond edges (for wave animation movement)
        const overflowX = 150
        const overflowY = 30

        // Calculate total lines needed to cover full width plus overflow on right side
        const totalLines = Math.ceil((width + overflowX) / xGap)
        const totalPoints = Math.ceil((height + overflowY) / yGap)

        // Start from 0 to ensure full left-to-right coverage
        const xStart = 0
        const yStart = -overflowY / 2
    ...
    return (
        ...
        >
            <svg
                ...
                style={{
                    maskImage: 'linear-gradient(to bottom, rgba(0,0,0,1) 0%, rgba(0,0,0,1) 50%, rgba(0,0,0,0.8) 70%, rgba(0,0,0,0.4) 85%, rgba(0,0,0,0) 100%)',
                    WebkitMaskImage: 'linear-gradient(to bottom, rgba(0,0,0,1) 0%, rgba(0,0,0,1) 50%, rgba(0,0,0,0.8) 70%, rgba(0,0,0,0.4) 85%, rgba(0,0,0,0) 100%)',
                }}
            />
            {/* Bottom fade overlay with blur effect */}
            <div 
                className="absolute bottom-0 left-0 right-0 h-1/3 pointer-events-none"
                style={{
                    background: 'linear-gradient(to bottom, transparent 0%, rgba(255,255,255,0.1) 30%, rgba(255,255,255,0.3) 60%, rgba(255,255,255,0.5) 100%)',
                    backdropFilter: 'blur(20px)',
                    WebkitBackdropFilter: 'blur(20px)',
                    maskImage: 'linear-gradient(to bottom, transparent 0%, rgba(0,0,0,0.3) 40%, rgba(0,0,0,0.7) 70%, rgba(0,0,0,1) 100%)',
                    WebkitMaskImage: 'linear-gradient(to bottom, transparent 0%, rgba(0,0,0,0.3) 40%, rgba(0,0,0,0.7) 70%, rgba(0,0,0,1) 100%)',
                }}
            />
        </div>
    )
